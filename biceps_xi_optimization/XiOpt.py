...
if __name__ == "__main__":
    outdir = "./"
    xi_values = np.linspace(1.0, 0.0, 11)
    print("Initial xi-values: ",xi_values)
    u_kln = np.load("fmo_u_kln.npy")
    new_xi_values,results = optimize_xi_values(xi_values, u_kln, outdir,
        nsteps=100000, tol=1e-7, alpha=1e-5, print_every=1000, make_plots=0, verbose=0)
        # NOTE: we used 50M steps for production, but 1M steps is faster
        # and gives same result. Other systems might need longer time to converge.
        #nsteps=5000000, tol=1e-7, alpha=1e-5, print_every=1000, make_plots=0, verbose=0)
    print("New xi-values: ",new_xi_values)
    exit()
